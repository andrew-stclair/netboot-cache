...
for location in config['locations']:
    ...
    NGINX_CONFIG += f"            set {config['locations'][location]} $UPSTREAM;\n"
    NGINX_CONFIG += f"            proxy_pass {config['locations'][location]};\n"
    NGINX_CONFIG += "            proxy_cache cache;\n"
    NGINX_CONFIG += "            proxy_read_timeout $PROXY_READ_TIMEOUT;\n"
    ...
    NGINX_CONFIG += "            $PROXY_CACHE_VALID\n"
    NGINX_CONFIG += "            add_header 'Access-Control-Allow-Origin' '$ALLOWED_ORIGIN';\n"
    NGINX_CONFIG += "        }\n"
